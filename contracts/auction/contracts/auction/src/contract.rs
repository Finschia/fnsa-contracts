...
use cosmwasm_std::{dynamic_link, from_binary, Addr, BankMsg, Binary, Coin, Contract, CosmosMsg, Deps, DepsMut, Empty, Env, MessageInfo, Response, StdResult, SubMsg, WasmMsg, to_binary, Uint128, Timestamp};
use cw2::set_contract_version;
use cw721::OwnerOfResponse;

use crate::error::ContractError;
use crate::msg::{ExecuteMsg, InstantiateMsg, QueryMsg, StartAuctionMsg, PlaceBidMsg, HighestBidResponse, AuctionItemResponse, AuctionHistoryResponse};
use crate::state::{Bid, Mode, State, History, STATE, BID, HISTORIES, HISTORY_INDEX};
...
pub fn start_auction(
    ...
    if msg.expiration_time > MAX_EXPIRATION_SECONDS {
        return Err(ContractError::ExpirationTimeError { val: msg.expiration_time });
    }

    // set start bid and seller
    BID.save(deps.storage, &Bid {
    ...
    let owner = from_binary::<OwnerOfResponse>(&contract.owner_of(msg.token_id.clone(), false)?)?.owner;
    if owner != info.sender {
        return Err(ContractError::Unauthorized {});
    }
    let is_success = contract.transfer_nft(info.clone(), env.contract.address.into_string(), msg.token_id.clone());
    if !is_success {
        return Err(ContractError::TransferNFTError { sender: info.sender, token_id: msg.token_id.clone() });
    }
...
pub fn place_bid(
    ...
    BID.update(deps.storage, |mut b| {
        if b.highest_bid >= msg.bid.clone() {
            return Err(ContractError::InvalidBidError { bid: msg.bid, highest_bid: b.highest_bid });
        }
...
pub fn end_auction(
    ...
    if bid.bidder != state.seller {
        ...
        let send_submsg: SubMsg<CosmosMsg> = SubMsg::reply_on_success(CosmosMsg::Bank(send_msg.clone()), 1u64);
    ...
    let send_submsg: SubMsg<CosmosMsg> = SubMsg::reply_on_success(CosmosMsg::Wasm(send_msg.clone()), 2u64);
    let _res: Response<CosmosMsg> = Response::new().add_submessage(send_submsg);

    // add auction history
    let idx = HISTORY_INDEX.load(deps.storage)?;
    HISTORIES.save(deps.storage, idx, &History { 
